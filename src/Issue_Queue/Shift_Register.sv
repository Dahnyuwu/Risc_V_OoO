...
// Output
    ...
    Register    #(.LENGTH(1))   SR_VA     (.clk(clk), .rst(arst),.in(valid_in),   .ena(s_ena), .out(valid_out));

endmodule


module Shift_Register_LW(
// Input
    input   logic           clk, rst, arst,
...
// Output
    ...
    Register    #(.LENGTH(32))  SR_IMM    (.clk(clk), .rst(rst), .in(imm_in),  .ena(s_ena), .out(imm_out));
    Register    #(.LENGTH(32))  SR_ADD    (.clk(clk), .rst(rst), .in(addr_in),  .ena(1'b1), .out(addr_out));
    Register    #(.LENGTH(1))   SR_Op     (.clk(clk), .rst(rst), .in(opcode_in),  .ena(s_ena), .out(opcode_out));
    Register    #(.LENGTH(6))   SR_RDT    (.clk(clk), .rst(rst), .in(rd_tag_in),  .ena(s_ena), .out(rd_tag_out));
    Register    #(.LENGTH(6))   SR_RS1T   (.clk(clk), .rst(rst), .in(rs1_tag_in), .ena(s_ena), .out(rs1_tag_out));
    Register    #(.LENGTH(6))   SR_RS2T   (.clk(clk), .rst(rst), .in(rs2_tag_in), .ena(s_ena), .out(rs2_tag_out));
    Register    #(.LENGTH(32))  SR_RS1D   (.clk(clk), .rst(rst), .in(rs1_data_in),.ena(u_ena), .out(rs1_data_out));
